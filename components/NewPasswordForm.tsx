...
export default function NewPasswordForm() {
  ...
  return <form onSubmit={submit} className="form-stack" noValidate>
    ...
    <label className="field"><span>Naujas slaptažodis</span><input name="password" type="password" required minLength={PASSWORD_MIN_LENGTH} maxLength={PASSWORD_MAX_LENGTH} autoComplete="new-password" aria-invalid={!!errors.password} aria-describedby="password-help" onChange={() => setErrors(current => ({ ...current, password: undefined }))} /><small id="password-help" className={errors.password ? 'password-error' : undefined} role={errors.password ? 'alert' : undefined}>{errors.password || `Mažiausiai ${PASSWORD_MIN_LENGTH} simboliai.`}</small></label>
    <label className="field"><span>Pakartoti slaptažodį</span><input name="confirmPassword" type="password" required minLength={PASSWORD_MIN_LENGTH} maxLength={PASSWORD_MAX_LENGTH} autoComplete="new-password" aria-invalid={!!errors.confirmPassword} aria-describedby={errors.confirmPassword ? 'confirm-error' : undefined} onChange={() => setErrors(current => ({ ...current, confirmPassword: undefined }))} />{errors.confirmPassword && <span id="confirm-error" className="password-error" role="alert">{errors.confirmPassword}</span>}</label>
